...
public class continuita_task : MonoBehaviour, InterfaceCollectible
{
    ...
    public Item_data ContinuitaData; //creo reference allo scriptable object coin -> in inspector assegno lo scriptable object (stile prefab)

    public void Collect()
    {
        Debug.Log("Collecting coin");
        Destroy(gameObject);
        onItemDataCollected?.Invoke(ContinuitaData);
    }
}
