...
public class illuminazione_task : MonoBehaviour, InterfaceCollectible 
{
    ...
    public Item_data LightData; //creo reference allo scriptable object coin -> in inspector assegno lo scriptable object (stile prefab)

    public void Collect()
    {
        Debug.Log("Collecting coin");
        Destroy(gameObject);
        onItemDataCollected?.Invoke(LightData);
    }
}
